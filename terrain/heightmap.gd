...
@onready var heightmap_rect: TextureRect = $texture
...
func _ready() -> void:
	randi()
	po2_dimensions = nearest_po2(size.x)
...
func compute_island_gpu(heightmap: Image) -> void:
	...
	if rd == null:
		push_warning("RenderingDevice is not available on the current rendering driver")
		return
...
func display_island(island: Image) -> void:
	...
	heightmap_rect.texture = island_tex
